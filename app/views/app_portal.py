...
async def channel_authorize(request: Request, channel_uuid: str):
    """Redirect to Google OAuth consent for channel authorization (web-based, no Playwright)."""
    user, redirect = require_user(request)
    if redirect:
        return redirect
    ...
    state = f"{channel_uuid}_{secrets.token_urlsafe(16)}"
    params = {
        "client_id": client_id,
...
async def oauth_callback(
    ...
    channel_uuid = parts[0] if parts else ""
    if not channel_uuid:
...
async def channel_detail(request: Request, channel_uuid: str):
    ...
    authorized = request.query_params.get("authorized") == "1"
    show_authorize_btn = bool(channel.get("console_id"))
    ...
    return templates.TemplateResponse("app_channel_detail.html", {
        ...
        "auth_error": auth_error, "authorized": authorized, "show_authorize_btn": show_authorize_btn,
    })
