...
Task Created (status=0 PENDING, scheduled_at set)
      ↓
Scheduler polls DB every 60s (scheduler/jobs.py)
      ↓
...
## Queue system (RQ)
Three queues in Redis:
- `publishing` — video uploads (30min timeout)
- `notifications` — Telegram/email notifications (2min timeout)
